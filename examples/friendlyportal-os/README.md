# FriendlyPortal OS (example image)

A complete, working **webhead** image you can copy and make your own. It's also
the binary's built-in default, so `webhead run` (no args) boots it.

```bash
cp -R examples/friendlyportal-os my-portal
...
```
friendlyportal-os/
  webhead.json              settings (name, ssid, domain, ports, motd, ssh login)
  data/
...
1. **Rename it.** In `webhead.json` change `name`, `ssid`, `hostname`, `prompt`,
...
5. **Test.** `webhead run my-portal`, browse <http://localhost:8080>, and
